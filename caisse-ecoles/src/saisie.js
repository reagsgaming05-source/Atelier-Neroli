...
(function () {
  ...
  for (const id of ['regYear', 'btnNewYear', 'regOpeningDate', 'regOpeningAmount', 'regCaisse', 'regInfo', 'btnRegOpenDir',
    ...
    'pMontant', 'pSensDebit', 'pSensCredit', 'pSensHint', 'pFiles', 'pFilesList', 'ficheErrors', 'btnPieceSave', 'btnPieceNew', 'btnPiecePreview', 'fichePreview', 'ficheFrame', 'btnPreviewClose',
  ...
  const state = { storage: null, reg: null, editingId: null, pending: [], years: [], previewUrl: null };
  ...
  function renderJournal() {
    ...
    els.journalBody.innerHTML = j.rows.map((r) => {
      ...
      return `<tr data-id="${r.id}"${state.editingId === r.id ? ' class="selected"' : ''}>` +
        ...
        `<td>${p && p.justificatifs.length ? `📎 ${p.justificatifs.length}` : ''}${p && p.source === 'scan' ? ' <span class="legend">scan</span>' : ''}</td>` +
  ...
  els.journalBody.addEventListener('click', async (ev) => {
    ...
    if (b.dataset.edit) {
      ...
      if (!p) return;
      state.editingId = p.id; state.pending = [];
      fillForm(p);
  ...
  window.CaisseSaisie = { state, init, openYear, addFromScan, renderJournal };
