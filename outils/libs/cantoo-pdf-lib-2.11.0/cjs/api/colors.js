...
const colorString = (color) => {
    ...
    return {
        rgb: (0, exports.rgb)(parsed.r, parsed.g, parsed.b),
        alpha: parsed.alpha,
    };
};
exports.colorString = colorString;
// prettier-ignore
const setFillingColor = (color) => color.type === ColorTypes.Grayscale ? (0, operators_1.setFillingGrayscaleColor)(color.gray)
    : color.type === ColorTypes.RGB ? (0, operators_1.setFillingRgbColor)(color.red, color.green, color.blue)
        : color.type === ColorTypes.CMYK ? (0, operators_1.setFillingCmykColor)(color.cyan, color.magenta, color.yellow, color.key)
            : (0, utils_1.error)(`Invalid color: ${JSON.stringify(color)}`);
exports.setFillingColor = setFillingColor;
// prettier-ignore
const setStrokingColor = (color) => color.type === ColorTypes.Grayscale ? (0, operators_1.setStrokingGrayscaleColor)(color.gray)
    : color.type === ColorTypes.RGB ? (0, operators_1.setStrokingRgbColor)(color.red, color.green, color.blue)
        : color.type === ColorTypes.CMYK ? (0, operators_1.setStrokingCmykColor)(color.cyan, color.magenta, color.yellow, color.key)
            : (0, utils_1.error)(`Invalid color: ${JSON.stringify(color)}`);
...
// prettier-ignore
const colorToComponents = (color) => color.type === ColorTypes.Grayscale ? [color.gray]
