...
exports.framize = framize;
exports.concatRGBA = concatRGBA;
const rgba_1 = require("./rgba");
function prepareDiff(cimg, w, h, nimg, rec) {
    (0, rgba_1.copyTile)(cimg, w, h, nimg, rec.width, rec.height, -rec.x, -rec.y, 2);
}
function updateFrame(bufs, w, h, frms, i, r, evenCrd) {
    ...
    if (frms[i - 1].dispose === 0) {
        (0, rgba_1.copyTile)(pimg, w, h, fr.img, nr.width, nr.height, -nr.x, -nr.y, 0);
        prepareDiff(cimg, w, h, fr.img, nr);
    }
    else {
        (0, rgba_1.copyTile)(cimg, w, h, fr.img, nr.width, nr.height, -nr.x, -nr.y, 0);
    }
}
/**
...
function framize(bufs, w, h, alwaysBlend, evenCrd, forbidPrev) {
    ...
    for (let j = 0; j < bufs.length; j++) {
        ...
        if (j !== 0) {
            const tlim = forbidPrev || alwaysBlend || j === 1 || frms[j - 2].dispose !== 0
                ? 1
                : 2;
            let tstp = 0;
            let tarea = 1e9;
            ...
            (0, rgba_1.copyTile)(pimg, w, h, nimg, nw, nh, -nx, -ny, 0);
            blend = (0, rgba_1.copyTile)(cimg, w, h, nimg, nw, nh, -nx, -ny, 3) ? 1 : 0;
            if (blend === 1) {
                prepareDiff(cimg, w, h, nimg, { x: nx, y: ny, width: nw, height: nh });
            }
            else {
                (0, rgba_1.copyTile)(cimg, w, h, nimg, nw, nh, -nx, -ny, 0);
            }
        }
        else {
...
//# sourceMappingURL=framize.js.map