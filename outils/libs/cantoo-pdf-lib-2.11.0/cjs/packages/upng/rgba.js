...
function decodeImage(data, w, h, out) {
    const area = w * h;
    const bpp = (0, filter_1.getBPP)(out);
        ...
        if (ts === undefined) {
            ...
            if (depth === 16) {
                for (let i = 0; i < area; i++) {
                    ...
                    bf32[i] =
                        (255 << 24) | (data[ti + 4] << 16) | (data[ti + 2] << 8) | data[ti];
                    if ((0, binary_1.readUshort)(data, ti) === tr &&
                        (0, binary_1.readUshort)(data, ti + 2) === tg &&
                        (0, binary_1.readUshort)(data, ti + 4) === tb) {
                        bf[qi + 3] = 0;
                    }
                }
            }
        ...
        for (let y = 0; y < h; y++) {
            ...
            if (depth === 1) {
                ...
                for (let x = 0; x < w; x++) {
                    ...
                    const al = (0, binary_1.readUshort)(data, off + (x << 1)) === tr ? 0 : 255;
                    bf32[to + x] = (al << 24) | (gr << 16) | (gr << 8) | gr;
                }
            }
        }
